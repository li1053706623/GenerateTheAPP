...
- (instancetype)dh_initWithFrame:(CGRect)frame imageNameArray:(NSArray<NSString *> *)imageNameArray buttonIsHidden:(BOOL)isHidden WithLeadType:(NSInteger)leadtype{
    if ([super initWithFrame:frame]) {
        ...
        for (int i=0; i<imageNameArray.count; i++) {
            ...
            if (leadtype == 0) {
                ...
                guidePageView.scrollEnabled = NO;
            }else{
                imageView.userInteractionEnabled = NO;
                guidePageView.scrollEnabled = YES;
            }
...
//                imageView.image = [UIImage imageNamed:imageNameArray[i]];
                [imageView sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@",imageNameArray[i]]]];
                [guidePageView addSubview:imageView];
            }
            
//            if (i<imageNameArray.count - 1) {
//                UIButton *skipButton = [[UIButton alloc]initWithFrame:CGRectMake(DDScreenW*0.8*i, DDScreenW*0.1, 50, 25)];
//                [skipButton setBackgroundImage:[UIImage imageNamed:@"exitbutton"] forState:UIControlStateNormal];
...
//            }
            ...
            if (i == imageNameArray.count-1 && isHidden == NO) {
               ...
               [startButton setBackgroundImage:[UIImage imageNamed:@"guideButton.jpg"] forState:UIControlStateNormal];
                [startButton.imageView setContentMode:UIViewContentModeScaleAspectFit];
...
- (instancetype)dh_initWithFrame:(CGRect)frame videoURL:(NSURL *)videoURL {
    if ([super initWithFrame:frame]) {
//        self.playerController = [[AVPlayerViewController alloc] initWithContentURL:videoURL];
...
-(void)NextPage{
    ...
    if (_currentIndex < self.imageArray.count - 1) {
        CGFloat x = (_currentIndex + 1) * self.guidePageView.frame.size.width;
        [self.guidePageView setContentOffset:CGPointMake(x, 0) animated:YES];
        self.imagePageControl.currentPage =_currentIndex + 1;
